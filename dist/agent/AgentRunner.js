...
function extractGermanDate(rawText) {
...
class AgentRunner {
    ...
    run(invoice) {
        ...
        for (const mem of vendorMemories) {
            ...
            if (mem.key === "serviceDate" &&
                ...
                if (extractedDate) {
                    ...
                    proposedCorrections.push(`serviceDate filled using vendor memory (${mem.value})`);
                    auditTrail.push({
                        step: "apply",
                        timestamp: new Date().toISOString(),
        ...
        return {
            ...
            confidenceScore: proposedCorrections.length > 0
